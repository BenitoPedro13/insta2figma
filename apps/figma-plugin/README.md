...
- **Histórico:** o ecrã inicial é a lista **History / Favorites** (persistido em **`figma.clientStorage`** no main thread via mensagens `history-request` / `history-save`; o `localStorage` do iframe **não** é fiável quando fechas o plugin). Após um import com sucesso, o username é adicionado ou actualizado; **★** marca favoritos. **Start Import** / **+** abrem o formulário; uma linha seleccionada pré-preenche o username. *(Avatares na lista: URL de `profilePicUrlHd` guardada no histórico após import com sucesso — se a CDN falhar ou expirar, vês a letra inicial. Ver [docs/PLUGIN_UI_DESIGN_SPEC.md §11](../../docs/PLUGIN_UI_DESIGN_SPEC.md#11-backlog-avatares-instagram-no-histórico).)*
