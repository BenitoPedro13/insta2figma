...
**Estado actual (após Fase 4):** NestJS em [`apps/api`](../apps/api) (`pnpm dev:api`) enfileira em **BullMQ** (`scrape-instagram-v1`); [`apps/worker`](../apps/worker) consome Redis e simula o scrape (transições `queued` → `running` → `succeeded` com `resultSummary` stub). Requer **Postgres + Redis** ([`docker-compose.yml`](../docker-compose.yml)), `REDIS_URL` em API e worker. **Próximo:** Fase 5 (scrape Instagram real). Prisma migrações em [`apps/api/prisma/migrations/`](../apps/api/prisma/migrations/).
